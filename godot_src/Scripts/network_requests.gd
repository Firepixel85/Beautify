...
@onready var base_url = (
	...
	else "https://bittifybackend.jyodann.workers.dev"
)
...
@onready var oauth_url = (
	"https://accounts.spotify.com/authorize?client_id=%s&response_type=code&redirect_uri=%s&scope=user-modify-playback-state user-read-currently-playing"
...
func check_for_sucess(res, return_raw: bool) -> Result:
	...
	if (
		result == HTTPRequest.Result.RESULT_CANT_RESOLVE
		|| result == HTTPRequest.Result.RESULT_CANT_CONNECT
		|| result == HTTPRequest.Result.RESULT_TIMEOUT
	):
		currently_online = false

		for i in http_request_nodes.values():
			i.cancel_request()
		return Result.new(false, {"error": "Cannot connect to the Internet"})
	currently_online = true
	var body_string = res[3] if (return_raw) else JSON.parse_string(res[3].get_string_from_utf8())

	# Server side Errors
	if response_code != 200:
		var server_error = {"response_code": response_code, "body_string": body_string}
		printerr("Server Error Occured")
		printerr(server_error)
		return Result.new(false, server_error)
	return Result.new(true, {"body_string": body_string})
