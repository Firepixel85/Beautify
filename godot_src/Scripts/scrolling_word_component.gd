...
func _ready():
	if bold != true:
		...
		song_name_label.remove_theme_font_size_override("font_size")
		duplicate_song_label.remove_theme_font_override("font")
		duplicate_song_label.remove_theme_font_size_override("font_size")
...
func _process(delta):
	...
	if is_animating:
		...
		if is_duplicate_following:
			...
			duplicate_song_label.position = Vector2(
				song_name_label.get_end().x + offset_between_text, song_name_label.position.y
			)

		else:
			duplicate_song_label.position += Vector2.LEFT * delta * speed_of_text

			song_name_label.position = Vector2(
				duplicate_song_label.get_end().x + offset_between_text,
				duplicate_song_label.position.y
			)
